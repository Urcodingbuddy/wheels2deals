...
import { ArrowRight, Wallet, CircleCheckBig, ShieldCheck, FileKey } from "lucide-react"
...
const services = [
  {
    ...
    icon: Wallet,
    title: "Finance & Loans",
    description: "UAE-wide assistance with expert support for locals and expats.",
    href: "/finance",
  },
  {
    number: "02",
...
export function ServicesRoofSection() {
  return (
    <section id="services" className="px-3.5 py-4">
      <div className="relative rounded-[24px] px-6 md:px-12 lg:px-16 py-16 md:py-20 overflow-hidden">
        ...
        {/* Header */}
        <div className="reveal relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-12 md:mb-16">
          <h2 className="font-[family-name:var(--font-display)] text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight">
...
function ServiceCard({
  ...
  return (
    ...
    >
      {/* Card number */}
      <span className="absolute top-6 right-6 text-white/10 text-5xl font-extrabold font-[family-name:var(--font-display)]">
        {number}
      </span>
      ...
      <div className="mb-6 w-12 h-12 rounded-xl bg-[#C9A84C]/10 flex items-center justify-center group-hover:bg-[#C9A84C]/20 transition-all duration-300 group-hover:shadow-[0_0_20px_rgba(201,168,76,0.3)]">
        <Icon className="w-6 h-6 text-[#C9A84C] group-hover:text-[#E8D5A3] transition-colors" strokeWidth={1.5} />
      </div>

      {/* Content */}
      <h3 className="font-[family-name:var(--font-display)] text-white font-bold text-lg mb-2">{title}</h3>
      <p className="font-[family-name:var(--font-body)] text-white/60 text-sm leading-relaxed mb-6">{description}</p>
