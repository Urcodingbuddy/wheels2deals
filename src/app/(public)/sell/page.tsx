...
import { 
  CheckCircle2, ChevronDown, Star, ShieldCheck, 
  CarFront, Banknote, Shield, Phone, Clock, ArrowRight, Plus 
} from "lucide-react";
        ...
        <div className="relative z-10 max-w-[1440px] mx-auto grid lg:grid-cols-[1.2fr_1fr] gap-16 items-center">
          ...
          <div className="flex flex-col items-start">
            ...
            <h1 className="font-[family-name:var(--font-display)] text-[clamp(44px,6vw,84px)] font-bold text-white leading-[1.05] tracking-tight mb-8">
              Sell smarter.<br />
              <span className="italic font-light text-[#C9A84C]">Sell for more.</span>
            </h1>
            <p className="font-[family-name:var(--font-body)] text-[18px] md:text-[20px] text-white/70 max-w-lg leading-relaxed mb-12">
              A premium UAE brokerage built for sellers who refuse to settle. We negotiate with verified buyers, handle every signature, and deliver the highest possible price — without the noise.
            </p>

            {/* Stats */}
            <div className="flex flex-wrap gap-8 md:gap-16 pt-8 border-t border-white/10 w-full max-w-lg">
              <div>
                <span className="font-[family-name:var(--font-display)] text-3xl font-bold text-white block mb-1">AED 10M+</span>
                <span className="font-[family-name:var(--font-body)] text-[12px] uppercase tracking-wider text-white/50">Cars Sold</span>
              </div>
              <div>
                <span className="font-[family-name:var(--font-display)] text-3xl font-bold text-white block mb-1">1000+</span>
                <span className="font-[family-name:var(--font-body)] text-[12px] uppercase tracking-wider text-white/50">Happy Sellers</span>
              </div>
              <div>
                <span className="font-[family-name:var(--font-display)] text-3xl font-bold text-white block mb-1 flex items-center gap-1">5.0 <Star className="w-5 h-5 fill-[#C9A84C] text-[#C9A84C]"/></span>
                <span className="font-[family-name:var(--font-body)] text-[12px] uppercase tracking-wider text-white/50">Avg. Rating</span>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                      ...
                      {brand.logo ? (
                        <img 
                          ...
                          className={`h-6 w-auto object-contain transition-all duration-300 ${
                            selectedBrand === brand.name ? 'opacity-100 drop-shadow-sm scale-110' : 'opacity-80 hover:opacity-100'
                          }`}
                        />
                      ) : (
                        <span className="h-6 flex items-center justify-center text-lg font-light text-[#2A3510]/40">...</span>
                      )}
                      <span className="text-[10px] font-semibold uppercase tracking-wider">{brand.name}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="relative">
                  ...
                  >
                    <span className={selectedYear ? "text-[#2A3510]" : "text-[#2A3510]/50"}>
                      {selectedYear || "Select"}
                    </span>
                    <ChevronDown className={`w-4 h-4 transition-transform duration-300 text-[#2A3510]/50 ${yearDropdownOpen ? 'rotate-180 text-[#C9A84C]' : ''}`} />
                  </button>
              ...
              >
                {isSubmitting ? "Calculating..." : formSuccess ? "We'll WhatsApp you ✓" : "Get market value →"}
              </button>
        ...
        <div className="max-w-[1440px] mx-auto">
          <div className="mb-16">
            ...
            <h2 className="font-[family-name:var(--font-display)] text-4xl md:text-5xl font-bold text-[#2A3510] tracking-tight mb-6 max-w-2xl">
              A four-step path to <span className="italic font-light">the right buyer.</span>
            </h2>
            <p className="font-[family-name:var(--font-body)] text-[18px] text-[#5A5A5A] max-w-2xl leading-relaxed">
              No marketplace ads. No tire-kickers. No back-and-forth haggling. Our broker model is engineered for serious sellers who value their time as much as their car.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
            ...
            {PROCESS_STEPS.map((step, idx) => (
              <div key={idx} className="relative z-10 bg-white/50 border border-[#2A3510]/5 rounded-[24px] p-8 hover:bg-white transition-colors duration-300 shadow-sm hover:shadow-md group">
        ...
        <div className="max-w-[1200px] mx-auto">
          <div className="mb-16 text-center">
            ...
            <h2 className="font-[family-name:var(--font-display)] text-4xl md:text-5xl font-bold text-[#2A3510] tracking-tight mb-6">
              Why brokerage <span className="italic font-light">beats</span> the alternatives.
            </h2>
            <p className="font-[family-name:var(--font-body)] text-[18px] text-[#5A5A5A] max-w-3xl mx-auto leading-relaxed">
              Most car-buying companies offer 15-25% below market value. Marketplace listings put you at the mercy of strangers. We sit between — fast when you need fast, premium when you want premium.
            </p>
          </div>
            ...
            <table className="w-full min-w-[800px] text-left border-collapse">
              <thead>
                <tr className="border-b border-[#2A3510]/10">
                  <th className="pb-6 font-[family-name:var(--font-body)] font-bold text-[#2A3510]/50 uppercase tracking-wider text-sm w-2/5">Channel</th>
                  <th className="pb-6 font-[family-name:var(--font-body)] font-bold text-[#2A3510]/50 uppercase tracking-wider text-sm">Speed</th>
                  <th className="pb-6 font-[family-name:var(--font-body)] font-bold text-[#2A3510]/50 uppercase tracking-wider text-sm">Price</th>
                  <th className="pb-6 font-[family-name:var(--font-body)] font-bold text-[#2A3510]/50 uppercase tracking-wider text-sm">Effort</th>
                </tr>
              </thead>
              <tbody>
                <tr className="border-b border-[#2A3510]/5 bg-[#C9A84C]/5">
                  <td className="py-8 pr-4">
                    <div className="font-[family-name:var(--font-display)] font-bold text-lg text-[#2A3510]">Wheels2Deals — <span className="text-[#C9A84C]">Private buyers</span></div>
                    <div className="text-sm text-[#5A5A5A] mt-1">Maximum value, full broker service</div>
                  </td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-[#2A3510]/20 text-sm font-semibold text-[#2A3510]/80 bg-white">Flexible · 7—14 days</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-[#C9A84C]/50 text-sm font-semibold text-[#C9A84C] bg-[#C9A84C]/10">★ Highest possible</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-green-600/30 text-sm font-semibold text-green-700 bg-green-50">Zero — we manage it</span></td>
                </tr>
                <tr className="border-b border-[#2A3510]/5 bg-[#2A3510]/5">
                  <td className="py-8 pr-4">
                    <div className="font-[family-name:var(--font-display)] font-bold text-lg text-[#2A3510]">Wheels2Deals — Dealer buyers</div>
                    <div className="text-sm text-[#5A5A5A] mt-1">Fast and convenient</div>
                  </td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-green-600/30 text-sm font-semibold text-green-700 bg-green-50">Same day</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-green-600/30 text-sm font-semibold text-green-700 bg-green-50">Strong market price</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-green-600/30 text-sm font-semibold text-green-700 bg-green-50">Low</span></td>
                </tr>
                <tr className="border-b border-[#2A3510]/5">
                  <td className="py-8 pr-4">
                    <div className="font-[family-name:var(--font-display)] font-bold text-lg text-[#2A3510]">Car-buying companies</div>
                    <div className="text-sm text-[#5A5A5A] mt-1">One-shot lowball offers</div>
                  </td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-green-600/30 text-sm font-semibold text-green-700 bg-green-50">Same day</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-red-500/30 text-sm font-semibold text-red-600 bg-red-50">15-25% below market</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-green-600/30 text-sm font-semibold text-green-700 bg-green-50">Low</span></td>
                </tr>
                <tr>
                  <td className="py-8 pr-4">
                    <div className="font-[family-name:var(--font-display)] font-bold text-lg text-[#2A3510]">Online marketplaces</div>
                    <div className="text-sm text-[#5A5A5A] mt-1">Listing yourself & dealing with strangers</div>
                  </td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-[#2A3510]/20 text-sm font-semibold text-[#2A3510]/80">Unpredictable</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-[#2A3510]/20 text-sm font-semibold text-[#2A3510]/80">Variable</span></td>
                  <td className="py-8 pr-4"><span className="px-4 py-2 rounded-full border border-red-500/30 text-sm font-semibold text-red-600 bg-red-50">High — calls, viewings, scams</span></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white rounded-[32px] p-10 md:p-14 shadow-lg border border-[#2A3510]/5 transition-transform hover:-translate-y-2 duration-300">
            ...
            <p className="font-[family-name:var(--font-body)] text-[16px] text-[#5A5A5A] mb-10 leading-relaxed">
              We'll share your car with hundreds of vetted dealers ready to buy this afternoon. Fast decision, simple paperwork, money in your account today.
            </p>
            <div className="pt-8 border-t border-[#2A3510]/10 flex items-baseline gap-4">
              <span className="font-[family-name:var(--font-display)] text-5xl font-bold text-[#C9A84C]">≤ 24h</span>
              <span className="text-[#5A5A5A] font-medium text-sm">to complete sale</span>
            </div>
          </div>
          ...
          <div className="bg-[#2A3510] rounded-[32px] p-10 md:p-14 shadow-2xl relative overflow-hidden transition-transform hover:-translate-y-2 duration-300">
            ...
            <p className="font-[family-name:var(--font-body)] text-[16px] text-white/70 mb-10 leading-relaxed">
              Patience pays. We professionally market your car to qualified private buyers and negotiate hard. Typically 12-18% more than dealer offers — without the headache.
            </p>
            <div className="pt-8 border-t border-white/10 flex items-baseline gap-4">
              <span className="font-[family-name:var(--font-display)] text-5xl font-bold text-[#C9A84C]">+18%</span>
              <span className="text-white/50 font-medium text-sm">average uplift</span>
            </div>
          </div>
        </div>
      </section>
            ...
            {FAQS.map((faq, i) => (
              <div key={i} className="border-b border-[#2A3510]/10 py-8">
                ...
                >
                  ...
                  <div className={`flex items-center justify-center flex-shrink-0 text-[#2A3510] transition-transform duration-300 ${openFaq === i ? 'rotate-45' : ''}`}>
      ...
      <section className="py-32 px-6 text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="font-[family-name:var(--font-display)] text-[clamp(40px,6vw,80px)] font-bold text-[#2A3510] leading-[1.05] tracking-tight mb-8">
            ...
            <span className="italic font-light text-[#C9A84C]">a better ending.</span>
          </h2>
          <p className="font-[family-name:var(--font-body)] text-[20px] text-[#5A5A5A] mb-12">
            Get a free, instant valuation. Take the next step in under two minutes.
          </p>
          <button 
